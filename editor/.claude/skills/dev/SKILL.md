# Pick and Complete Next Task
1. Pick up a ticket to work on by running `nbd next`
2. Implement the changes across all necessary files
3. Run `cargo build` to verify compilation
4. Run `cargo test` if tests exist
5. Mark the task as complete
6. Update PLANNING.md to reflect the status of the project and what was done in this working session.
7. If any follow-up work needs to be done, add it to PLANNING.md and create additional beads tasks
8. Commit with a descriptive message referencing the task ID
